...
const PropertyCard = ({ address, title, rooms, bathrooms, area, price, type, agentName, imageUrl, onClick }) => {
  ...
  const getImageUrl = () => {
    ...
    if (imageUrl.startsWith("/")) {
      return `http://localhost:10101/${imageUrl}`
    }
...
export const Main = () => {
  ...
  useEffect(() => {
    const fetchProperties = async () => {
      ...
      try {
        ...
        const res = await fetch("http://localhost:10101/api/properties/with-images")
        if (!res.ok) {
  ...
  const handleOperationTypeClick = async (operationType) => {
    ...
    try {
      ...
      const url = `http://localhost:10101/api/search/search?${queryParams.toString()}`
